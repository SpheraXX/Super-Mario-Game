...
std::unique_ptr<Projectile> Lakitu::createProjectile() {
    ...
    float lead = 0.0f;
    // findPlayer() hands back an Entity; velocity is a Character capability (see Entity.h),
    // so ask for it as one. The player is always a Character, but a failed cast simply
    // means no lead — the egg drops straight down rather than crashing.
    if (const auto* target = dynamic_cast<const Character*>(findPlayer())) {
