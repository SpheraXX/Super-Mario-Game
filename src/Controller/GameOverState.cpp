...
GameOverState::GameOverState(std::function<void()> onRestartCallback)
    : m_onRestartCallback(std::move(onRestartCallback)), 
      m_buttonList(view::ui::UIContainer::Layout::Vertical, view::ui::layout::MenuButtonGap) {
}
...
void GameOverState::buildUI() {
    ...
    m_titleLabel = view::ui::UILabel(font, "GAME OVER", view::ui::layout::TitleFontSize * 2, sf::Color(230, 60, 60));
    m_titleLabel.setCentered(true);
    
    int score = model::GameManager::instance().getScore();
    m_scoreLabel = view::ui::UILabel(font, "Final Score: " + std::to_string(score), view::ui::layout::ButtonFontSize, sf::Color::White);
    ...
    auto makeBtn = [&](const std::string& label, std::function<void()> cmd) {
        ...
        btn->setColors(view::ui::theme::ColorNormal, view::ui::theme::ColorHovered, view::ui::theme::ColorText);
        btn->setOnClick(std::move(cmd));
        m_buttonList.add(std::move(btn));
    };

    makeBtn("RESTART", [this]() {
        if (m_onRestartCallback) m_onRestartCallback();
    });
    ...
    m_slideIn = view::effect::LerpAnimator(H, H * 0.75f, 0.6f, view::effect::Easing::OutQuad);
...
void GameOverState::onDisplayModeChanged() {
    ...
    m_titleLabel.setSize(W, static_cast<float>(view::ui::layout::TitleFontSize) * 4.f);
    m_titleLabel.setPosition(0.f, H * 0.35f);
    
    m_scoreLabel.setSize(W, static_cast<float>(view::ui::layout::ButtonFontSize) * 2.f);
