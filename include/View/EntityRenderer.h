...
public:
    ...
    virtual void render(sf::RenderWindow& window, const model::Entity& entity) const = 0;
};
...
public:
    void render(sf::RenderWindow& window, const model::Entity& entity) const final {
        renderTyped(window, static_cast<const T&>(entity));
    }

protected:
    virtual void renderTyped(sf::RenderWindow& window, const T& entity) const = 0;
};
